...
struct audio_buffer {
	unsigned len, len_period;
	unsigned period;
	u16 pos_period[NR_PERIODS];
...
{
	abuf->len = len;
	abuf->len_period = len / NR_PERIODS;
	...
	abuf->lock_compl = __SPIN_LOCK_UNLOCKED(x);
...
static void audio_buffer_dma_cb(void *userp)
{
	...
	abuf->pos_period[abuf->period] = 0;
	abuf->period = (abuf->period + 1) % NR_PERIODS;
			 ...
			 size_t count, loff_t *offp)
{
	...
	int next_usable_period = -1;
	unsigned period_snapshot;
	u16 *pos_period;
	unsigned copy_len, user_pos = 0, user_left = count;
	void *dst;
	int ret, j;
	...
	//while (user_pos < count) 
	{
	retry_find_period:
		period_snapshot = abuf->period + 1;

		for (unsigned i=0; i<NR_PERIODS; i++) {
			j = (period_snapshot + i) % NR_PERIODS;
			if (abuf->pos_period[j] < abuf->len_period) {
				next_usable_period = j;
				break;
			}
		}

		if (next_usable_period == -1) {
			reinit_completion(&abuf->compl_buf_period);

			if (wait_for_completion_interruptible(&abuf->compl_buf_period) < 0)
				printk("DMA operation cancelled\n");

			goto retry_find_period;
		}

		pos_period = &abuf->pos_period[next_usable_period];
		copy_len = abuf->len_period - (*pos_period);
		dst = abuf->buf + abuf->len_period * next_usable_period + (*pos_period);

		if (user_left < copy_len)
			copy_len = user_left;

		if (copy_from_user(dst, buf + user_pos, copy_len))
			return -EFAULT;
...
static int x1501_audio_dev_probe(struct platform_device *pdev)
{
	...
	clk_ext = devm_clk_get(dev, "ext");
	if (IS_ERR(clk_ext))
		return PTR_ERR(clk_ext);

	ret = clk_set_parent(priv.clk_i2s, clk_ext);
	if (ret) {
		dev_err(dev, "failed to set I2S parent to EXCLK\n");
		return ret;
	}
